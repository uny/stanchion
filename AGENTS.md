...
`src-tauri/capabilities/default.json`. Do only the second and the build fails, because
`validate_capabilities` rejects an identifier no manifest defines. Do only the first and
nothing fails until the new command is first invoked, which is the dangerous direction: the
window still renders "core 0.0.0", because the frontend only ever invokes `core_version` and
that one is still granted, and CI stays green because nothing in CI launches the app. Only
the feature that needs the new command finds out. Every such addition widens what the WebView can reach and must say so in the pull
